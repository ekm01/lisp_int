...
typedef struct IsStringRet {
  int isString;
  char *val;
} IsStringRet;

char *pretokenize(char *program); // insert spaces around parentheses and
                                  // operators, returns new program

TokenList tokenize(char *program,
                   unsigned int length); // tokenizes the program text, returns
                                         // the tokenlist and size
